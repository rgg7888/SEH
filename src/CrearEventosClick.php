...
class CrearEventosClick {
    ...
    public function obtenerDatosGetSuperSimple(string $url, string $script) {
        ...
        $peticion = "const xhr = new XMLHttpRequest();"."xhr.open(".htmlspecialchars("'GET'_'".$url."');")."xhr.send();";
        $js = '';
    ...
    public function escribirScript() {
        ...
        for($i = 0; $i < $numeroDeEventos; $i++) {
            array_push($script, "_const ".$this->obtenerBotones()[$i]." = document.getElementById('".$this->obtenerIds()[$i]."');" );
