...
class CrearEventosClick {
    ...
    public function obtenerDatosGetSuperSimple(string $url, string $script) {
        ...
        $peticion = "const xhr = new XMLHttpRequest();"."xhr.open("."'GET'&comma;'".$url."');"."xhr.send();";
        $js = '';
    ...
    public function escribirScript() {
        ...
        for($i = 0; $i < $numeroDeEventos; $i++) {
            ...
            array_push($script, $this->obtenerBotones()[$i].".addEventListener('click'&comma;".$this->obtenerFunciones()[$i].");" );
        }
